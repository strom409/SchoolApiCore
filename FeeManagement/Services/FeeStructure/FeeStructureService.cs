    ...
    public class FeeStructureService : IFeeStructureService
    {
        ...
        public async Task<ResponseModel> AddFeeStructure(FeeStructureDto dto, string clientId)
        {
            ...
            try
            {
                ...
                string dupQuery = @"SELECT COUNT(*) FROM FeeStructure 
                            WHERE CIDFK = @CIDFK AND Title = @Title AND IsDeleted = 0";
                var dupParams = new[]
                {
            ...
            new SqlParameter("@Title", dto.Title ?? string.Empty)
                ...
                if (exists > 0)
                {
                    response.IsSuccess = false;
                    response.Status = 0;
                    response.Message = "FeeStructure with same Class and Title already exists!";
                    return response;
                }
                #endregion
        ...
        public async Task<ResponseModel> GetAllFeeStructures(string clientId, string currentSession)
        {
            ...
            try
            {
                ...
                #region Execute Select Query with Current Session
                string query = @"
            SELECT * 
            FROM FeeStructure 
            WHERE IsDeleted = 0 AND CSession = @CurrentSession";

                var sqlParams = new[]
                {
            new SqlParameter("@CurrentSession", currentSession ?? string.Empty)
        };
        ...
        public async Task<ResponseModel> GetFeeStructureByClassAndFeeHead(long classId, long fhId, string clientId)
        {
            ...
            var response = new ResponseModel
            {
                IsSuccess = true,
                Status = 0,
                Message = "FeeStructure not found."
            };
            #endregion
            ...
            try
            {
                ...
                #region Execute Select By ClassID & FHIDFK
                string query = @"
            SELECT * 
            FROM FeeStructure 
            WHERE CIDFK = @CIDFK 
              ...
              AND IsDeleted = 0";
                ...
                var sqlParams = new[]
                {
            new SqlParameter("@CIDFK", classId),
            new SqlParameter("@FHIDFK", fhId)
        };
            ...
            catch (Exception ex)
            {
                ...
                Repository.Error.ErrorBLL.CreateErrorLog("FeeStructureService", "GetFeeStructureByClassAndFeeHead", ex.ToString());
                return response;
            }
        }
